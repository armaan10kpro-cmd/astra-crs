...
def test_parse_cpp_stack_frame_with_column():
    stderr = "#0 0x4011ab in process_input src/parser.cpp:42:12"
    frame = parse_stack_frame(stderr)
    assert frame is not None
    assert frame["function"] == "process_input"
    assert frame["source_file"] == "src/parser.cpp"
