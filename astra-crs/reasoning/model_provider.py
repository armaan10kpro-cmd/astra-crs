...
class LocalProvider(ModelProvider):
    ...
    def __init__(self, endpoint: str | None = None, model: str | None = None):
        ...
        self.model = model or os.environ.get("ASTRA_LOCAL_MODEL", "qwen2.5-coder:7b")

    def complete(self, prompt: str, *, system: str = "") -> str:
        import urllib.request

        endpoints = [self.endpoint, "http://127.0.0.1:11434", "http://localhost:11434"]
        last_error = None
        ...
        raise RuntimeError(f"LocalProvider ({self.model}) unavailable across endpoints {endpoints}: {last_error}")
