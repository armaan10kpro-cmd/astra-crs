...
def run_pipeline(
    *,
    ...
    discovery = run_discovery(root, binary=target_binary)
    # Determine the path to the binary for runtime protection.
    # When no explicit --target is supplied, use the canonical controlled
    # lab binary used by the discovery pipeline.
    if target_binary is not None:
        protected_binary = target_binary
    elif target_config is not None:
        protected_binary = target_config.abs_binary_path(root)
    else:
        protected_binary = root / "targets/demo_app/demo_vuln"
